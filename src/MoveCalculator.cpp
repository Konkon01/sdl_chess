...
void MoveCalculator::calculate_king(int f_ind, char f_piece, std::set<int> &p_moves){
  ...
  for (int i = 0; i < 64; i++){
    ...
    if( (current_p == 'k' || current_p == 'K') && !is_same_team(f_piece, current_p)){
      basic_king_calculation(i, current_p, opp_moves);

    } else if (current_p != 'x' && !is_same_team(board->board[i], f_piece)){
...
void MoveCalculator::basic_king_calculation(int f_ind, char f_piece, std::set<int> &p_moves){
  if(board->board[f_ind - 9] == 'x' || is_same_team(f_piece, board->board[f_ind - 9])){
    p_moves.insert(f_ind - 9);
  }
  if(board->board[f_ind - 8] == 'x' || is_same_team(f_piece, board->board[f_ind - 8])){
    p_moves.insert(f_ind - 8);
  }
  if(board->board[f_ind - 7] == 'x' || is_same_team(f_piece, board->board[f_ind - 7])){
    p_moves.insert(f_ind - 7);
  }
  if(board->board[f_ind - 1] == 'x' || is_same_team(f_piece, board->board[f_ind - 1])){
    p_moves.insert(f_ind - 1);
  }
  if(board->board[f_ind + 1] == 'x' || is_same_team(f_piece, board->board[f_ind + 1])){
    p_moves.insert(f_ind + 1);
